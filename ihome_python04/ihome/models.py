...
class BaseModel(object):  # django中也是此类相似的做法
...
class User(BaseModel, db.Model):
    ...
    @password.setter
    def password(self, origin_password):  # 使用装饰器
        """
        设置属性
        :param  origin_password  原来的明文密码
        """
        # 对密码进行hash加密计算
        self.password_hash = generate_password_hash(origin_password)

    def check_password(self, passwd):  # 用户登录时填写的原始密码
        # 检验密码的正确性
